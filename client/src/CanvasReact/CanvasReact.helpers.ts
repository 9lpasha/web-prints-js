...
/** Рисование стрелки уже созданной */
...
export const drawGrid = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
  ...
  const left = (Math.ceil(canvasWindowOptions.min.x / canvasQuadroSize) * canvasQuadroSize - canvasWindowOptions.min.x) * scale;
  const top = (Math.ceil(canvasWindowOptions.min.y / canvasQuadroSize) * canvasQuadroSize - canvasWindowOptions.min.y) * scale;

  // Рисуем вертикальные линии
  for (let x = left; x <= width; x += canvasQuadroSize * scale) {
    ctx.beginPath();
    ctx.moveTo(x, 0);
    ctx.lineTo(x, height);
    ctx.stroke();
  ...
  for (let y = top; y <= height; y += canvasQuadroSize * scale) {
    ctx.beginPath();
    ctx.moveTo(0, y);
    ctx.lineTo(width, y);
    ctx.stroke();
